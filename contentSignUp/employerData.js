...
database.ref('/usersDatas/').on('value', function (snapshot) {
  ...
  for (let i = 0; i < users.length; i++) {
    ...
    if (users[i] != null) {
      // $('#row').append('<div class="col-md-3"></div>').attr('id',i)

      // $('#freelancerDatas ').append('<div  >' + users[i].username + ' ' + users[i].password + '</div>')
      // $('#freelancerDatas ').append('<div  >' + users[i].pay + ' ' + users[i].money + '</div>')
      // $('#freelancerDatas ').append('<div  >' + users[i].category + ' ' + users[i].languages + '</div>')
      // $('#freelancerDatas ').append('<div  ><button type="submit">Apply Job </button></div>')
